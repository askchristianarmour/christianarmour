...
export function RevokePermissionConfirmationModal({
  open,
  email,
  isRevoking = false,
  onClose,
  ...
  return createPortal(
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="revoke-permission-title"
      ...
      >
        ...
        <div className="flex items-start gap-3 pr-8">
          ...
          <div>
            <h2 id="revoke-permission-title" className="text-xl font-semibold text-slate-900">
              Remove user?
            </h2>
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Remove{' '}
              <span className="font-semibold text-slate-800">{email}</span> from the permissions
              list? They will no longer be able to add or edit articles.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row-reverse">
          ...
          >
            {isRevoking ? 'Removing...' : 'Yes, remove user'}
          </button>
          <button
            type="button"
            onClick={onClose}
